...
namespace PR2_Level_Generator
{
	public class GenSimpleRace : ILevelGenerator
	{
		...
		private List<Point> GetNewFinishables(Point p)
		{
			...
			if (b.T == BlockID.BLANK || b.T == BLOCK_FINISHABLE)
			...
			if (b.T == BlockID.BLANK || b.T == BLOCK_FINISHABLE)
			{
				...
				if (b2.T == BlockID.BLANK || b2.T == BLOCK_FINISHABLE) // left-up is blank
				...
				else
				{
					...
					else if (Easy != 1)
					{
						...
						if (b2.T == BlockID.BLANK || b2.T == BLOCK_FINISHABLE) // two spaces left is open
						{
							b2 = Map.GetBlock(p.X - 2, p.Y - 1);
							if (b2.T == BlockID.BLANK || b2.T == BLOCK_FINISHABLE) // two spaces left and one up is open
								ret.Add(new Point(p.X - 1, p.Y));
						}
					}
				}
			...
			{
				b2 = Map.GetBlock(p.X + 1, p.Y - 1);
				if (b2.T == BlockID.BLANK || b2.T == BLOCK_FINISHABLE)
					ret.Add(new Point(p.X + 1, p.Y));
				else
				{
					b2 = Map.GetBlock(p.X + 1, p.Y + 1);
